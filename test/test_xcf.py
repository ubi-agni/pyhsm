...
if __name__ == '__main__':
    ...
    sim.xcf_subscribe(mi_sim, xpath="/EVENT[@sender='ArmServer' and @name='finished' and @id='done']",
                      handler="finished")
    sim.add_transition("finished", real)

    # if real command succeeded, transition to done state
    real.xcf_subscribe(mi_real, xpath="/EVENT[@sender='ArmServer' and @name='finished' and @id='done']",
                      handler="finished")
    real.add_transition("finished", done)

    # subscribe to ArmServer:errors on top-level of state machine for simplicity
    m.xcf_subscribe(mi_sim, xpath="/EVENT[@sender='ArmServer' and @name='error' and not(@interrupted)]",
                    handler="ArmServer:error")
    m.xcf_subscribe(mi_real, xpath="/EVENT[@sender='ArmServer' and @name='error' and not(@interrupted)]",
                    handler="ArmServer:error")
