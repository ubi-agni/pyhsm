...
class ActionState(hsm.Container):
    ...
    def __init__(self, action, spec=None, goal=None, name=None, server_wait_timeout=10.0):
        ...
        self._action_wait_thread = threading.Thread(
            name=self._action_name + "/wait_for_server", target=self._wait_for_server
        )
    ...
    def _wait_for_server(self):
        ...
        while (
            (self._status == ActionState.WAITING_FOR_SERVER or self._status == ActionState.PENDING)
            ...
            try:
                ...
                raise
        _LOGGER.warning(
            "Action server '{0}' not found within {1:.1f} seconds.".format(
                self._action_name,
                (rospy.get_rostime() - timeout_time + self._server_wait_timeout).to_sec(),
            )
        )
    ...
    def _send_goal(self):
        ...
        self._action_client.send_goal(
            self.goal, self._goal_done_cb, self._goal_active_cb, self.feedback_cb
        )
    ...
    def _goal_done_cb(self, result_state, result):
        ...
        _LOGGER.debug(
            "Action {} sent result {}({})".format(
                self._action_name, get_result_str(result_state), result_state
            )
        )
